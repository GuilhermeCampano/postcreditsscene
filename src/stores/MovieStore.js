...
class MovieStore extends EventEmitter {
	...
	handleActions(action) {
		switch (action.type) {
			case "GET_MOVIES":
				...
				break;
			case "VOTE_POLL":
				this.insertOrUpdateMovie(action.value.movie, action.value.voteType);
				break;
			default:
				break;
		}
	...
	insertOrUpdateMovie = (moviePayload, voteType) => {
		if (!moviePayload) {
			return false;
		}

		moviePayload = this.parsePostCredits(moviePayload, voteType);
	...
	parsePostCredits(movie, voteType) {
		const votePollResult = VotePollStore.setMovie(movie.id, voteType);
		let postCredits = {
			yes: 0,
			no: 0
		};
		switch (votePollResult) {
			case 'CREATE_YES':
				postCredits.yes++;
				break;
			case 'CREATE_NO':
		...
		movie.post_credits = postCredits;
		return movie;
	}
	getMovies = (query) => {
